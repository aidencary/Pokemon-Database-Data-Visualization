...
public class TablePanel extends JPanel {
    ...
    private void applyFilters() {
        ...
        sorter.setRowFilter(new RowFilter<DefaultTableModel, Integer>() {
            ...
            public boolean include(Entry<? extends DefaultTableModel, ? extends Integer> entry) {
                ...
                boolean isLegendary = Boolean.parseBoolean(entry.getValue(9).toString());
                String generation = (String) entry.getValue(10).toString();
    ...
    private void initializeTableModel() {
        String[] columns = {"Name", "Type 1", "Type 2", "HP", "Attack", "Defense", "Sp. Atk", "Sp. Def", "Speed", "Generation", "Legendary"};
        ...
        hideColumn(9); // Generation
        hideColumn(10); // Legendary
    }

    // Helper method to hide columns
    ...
    private void populateTable(List<Pokemon> pokemonList) {
        for (Pokemon p : pokemonList) {
            model.addRow(new Object[]{p.name(), p.type1(), p.type2(), p.hp(), p.attack(), p.defense(), p.spAtk(), p.spDef(), p.speed(), p.generation(), p.legendary()});
        }
    }
...
public class TablePanel extends JPanel {
    ...
    private void setupSelectionListener() {
        table.getSelectionModel().addListSelectionListener(e -> {
            if (!e.getValueIsAdjusting() && table.getSelectedRow() != -1) {
                int row = table.convertRowIndexToModel(table.getSelectedRow());
                Pokemon selectedPokemon = fullPokemonData.get(row);
                System.out.println("Selected Pokémon: " + selectedPokemon);
            }
        });
    }
